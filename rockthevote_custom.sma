...
public rockthevote(id)
{
	...
	if	((contain(said, "/rockthevote") != -1) || (contain(said, "rockthevote") != -1) || (contain(said, "rtv") != -1))
	{
		if (get_gametime() < (get_pcvar_float(timevote) * 60.0))
			client_print(id, print_chat, "Atualmente, não é permitido votar. Esperar %d minutos.", (floatround(((get_pcvar_float(timevote) * 60.0) - get_gametime()) / 60.0)));
		else
		{
			if	(rtv[id-1] == id)
			...
			else
			{
				...
				client_print ( 0, print_chat, "%s Voto foi adicionado. Necessita %d ou %d^%^%  Para começar a votar!", name, (get_pcvar_num(howmanyvotes)-count), (floatround(get_pcvar_float(howmanyvotesperc) * 100.00)))
			}
		}
	}
	return PLUGIN_CONTINUE;
...
bool:RetrieveMaps(s_MapsFound[][])
{
	...
	switch (i_MapsCount)
    {
        case 0             : log_amx("Nao existem mapas no %s.", s_File);
        case 1 .. MAX_MAPS : log_amx("Nao foram encontrados mapas suficientes. (requires at least %d maps)", MAX_MAPS + 1 );
