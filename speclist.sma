...
public cmdSpecList(id)
{
	if( gOnOff[id] )
	{
		client_print(id, print_chat, "[Only Dead] Você não verá mais quem está se encontrando com você.");
		gOnOff[id] = false;
	}
	else
	{
		client_print(id, print_chat, "[Only Dead] Agora você vai ver quem está te observando.");
		gOnOff[id] = true;
	}
...
public tskShowSpec()
{
	...
	for( new alive = 1; alive <= gMaxPlayers; alive++ )
	{
		...
		format(szHud, 45, "Spectadores %s:^n", szName);
